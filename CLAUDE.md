...
npm run build        # Production build (apps/web + apps/api)
npm run lint         # ESLint check (apps/web + apps/api)
```

To run workspace-scoped commands directly:
```bash
...
Full local flow without Docker: run `npm run dev -w apps/web` (3000) and `npm run dev -w apps/api` (3001) in parallel — `apps/web/next.config.mjs` proxies `/api/:path*` to `:3001` in dev, so the whole flow works through `http://localhost:3000` alone. Via Docker: `npm run infra:up` (`docker compose up -d --build`), available at `http://localhost` (port 80, via Nginx).

There are no test commands configured yet. Check `.claude/specs/` for specs that may include planned test setup.

## Architecture

**Monorepo** (npm workspaces):
- `apps/web` — Next.js 14+ (App Router) frontend only (pages, form, `middleware.ts` for the LGPD banner). No AWS access.
- `apps/api` — Next.js 14+ (Route Handlers only, no UI): `/api/session`, `/api/draft`, `/api/upload-url`, `/api/submit`, `/api/aceite-termo`, plus `lib/auth.ts`, `lib/rateLimit.ts`, `lib/aws/*`. **Not exposed publicly in production** — its port (3001) is published in `docker-compose.yml`/`docker-compose.prod.yml` for local debug, but the production server firewall blocks external access to it; only the Nginx (and same-host traffic) can reach it.
- `apps/worker` — [Planned Phase 7] AWS Lambda SQS consumer (PDF generation, SES emails)
- `packages/shared` — `@prolink/shared`: Zod schemas and TypeScript types shared between web, api, and worker
...
5. Worker Lambda generates PDF and sends confirmation email via SES
...
- `packages/shared/src/schemas/` — Zod validation schemas (source of truth for form data shape)
...
**Shared package import:** use `@prolink/shared` (workspace-linked, no build step needed in dev).
